...
\begin{itemize}
    ...
    \item \textbf{Asignación ineficiente de recursos de postventa.} El equipo de Customer Success suele intervenir tarde y con ofertas poco ajustadas al perfil del cliente en riesgo. No existe una clasificación operativa de riesgo durante la etapa activa (más allá del Onboarding), por lo que muchas acciones se disparan cuando la probabilidad de abandono ya es alta. Se termina priorizando la contención de clientes al borde del churn y se desaprovechan intervenciones tempranas con mayor potencial de retención o expansión.
\end{itemize}
...
La tasa de bajas mensuales se estima en torno a 2{,}4\,\%. Con este nivel, la empresa pierde alrededor de 80 clientes al mes, lo que equivale a cerca de 1.000 clientes en un año. En términos de ingresos, esta dinámica se traduce en una pérdida anual cercana a \$59.000.000 de pesos en suscripciones que dejan de cobrarse. Cada décima de punto porcentual de reducción en la tasa de bajas representa del orden de \$2.500.000 de pesos anuales en ingresos conservados, lo que entrega una referencia directa sobre cuánto vale económicamente mejorar la retención.

El proyecto busca reducir estas pérdidas mediante un uso más focalizado de los recursos comerciales. El modelo permite ordenar la cartera según probabilidad de baja y entregar al equipo de Customer Success un listado priorizado, de modo que se concentren los esfuerzos en quienes generan mayor riesgo económico. Si el uso del modelo y de las políticas asociadas permitiera reducir la tasa mensual desde 2{,}4\,\% a 2{,}0\,\%, la empresa dejaría de perder cerca de \$10.000.000 de pesos al año en ingresos por suscripciones, solo por efecto de una menor cantidad de clientes que se dan de baja. La segmentación por comportamiento también abre espacio para acciones de expansión comercial: un subconjunto de clientes puede estar en condiciones de aumentar de plan o contratar servicios adicionales y, si una fracción pequeña de la cartera incrementa su pago mensual, el ingreso adicional anual puede ser comparable al monto ahorrado por una mejor retención. Los detalles de las métricas, las fórmulas y los supuestos que permiten cuantificar el valor económico por cliente y proyectar escenarios más completos de ingresos se desarrollan en el marco conceptual y en el capítulo metodológico, siguiendo la literatura de analítica de clientes y gestión basada en valor \citep{Kumar2010,Gupta2006,Chapman2000}.
...
La quinta tarea cuantificó una línea base económica de la cartera. Se fijó \(N_{\mathrm{activos}} \approx 3{.}500\) y un ingreso anual recurrente cercano a \(2{.}465{.}400{.}000\), lo que da \(\mathrm{MRR}_{\mathrm{base}} \approx 205{.}450{.}000\) y un ingreso promedio por cuenta \(\mathrm{ARPA} \approx 58{.}700\) (anexo ~ \ref{tab:mensualidad_por_rubro}). Con una tasa de bajas mensuales \(\mathrm{Churn}_{\mathrm{base}} = 0{,}024\) se estimó que se pierden cerca de \(N_{\mathrm{bajas}} \approx 84\) clientes al mes y un ingreso \(\mathrm{MRR}_{\mathrm{perdido}} \approx 4{.}930{.}800\), equivalente a unos \(59{.}200{.}000\) al año. 

A partir de esta base se evaluaron escenarios de mejora usando la expresión de ingreso retenido incremental. Reducir la tasa de bajas desde \(0{,}024\) a \(0{,}020\) implica \(\Delta \mathrm{Churn} = 0{,}004\) y un \(\Delta \mathrm{MRR}_{\mathrm{retenido}} \approx 821{.}800\) mensuales, es decir cerca de \(9{.}861{.}600\) al año. Una mejora de \(0{,}001\) punto porcentual en la tasa de bajas representa \(\Delta \mathrm{MRR}_{\mathrm{retenido}} \approx 205{.}450\) al mes, unos \(2{.}465{.}400\) al año. Estas cifras entregan el orden de magnitud del problema económico asociado al churn y servirán para interpretar los resultados del modelo y de las políticas de retención y expansión.
...
Las variables se calculan en el script principal y se documentan en el datamart. El panel contiene once columnas: \texttt{id\_\hspace{0pt}cliente}, \texttt{mes\_\hspace{0pt}corte}, \texttt{pais}, \texttt{rubro}, \texttt{plan\_\hspace{0pt}nombre}, \texttt{arpa\_\hspace{0pt}proxy}, \texttt{tenure\_\hspace{0pt}meses}, \texttt{citas\_\hspace{0pt}30d}, \texttt{recencia\_\hspace{0pt}uso\_\hspace{0pt}dias}, \texttt{bloqueos\_\hspace{0pt}90d} y \texttt{churn\_\hspace{0pt}T1}.

\texttt{id\_\hspace{0pt}cliente} identifica la cuenta y \texttt{mes\_\hspace{0pt}corte} es el primer día del mes de referencia. \texttt{pais}, \texttt{plan\_\hspace{0pt}nombre} y \texttt{rubro} provienen de las tablas comerciales y describen el contexto. \texttt{arpa\_\hspace{0pt}proxy} se toma desde \texttt{mensualidad}, reemplazando ceros por nulos con \texttt{NULLIF\hspace{0pt}(mensualidad,0)}, y \texttt{tenure\_\hspace{0pt}meses} mide la antigüedad con \texttt{TIMESTAMPDIFF(MONTH, fecha\_\hspace{0pt}creacion, mes\_\hspace{0pt}corte)}.

Las variables de uso reciente se construyen agrupando citas por cliente en \texttt{appointment\_\hspace{0pt}ticket} y \texttt{cliente\_\hspace{0pt}servicio}. \texttt{citas\_\hspace{0pt}30d} cuenta las citas en los 30 días previos al corte y toma valor cero si no hay registros; \texttt{recencia\_\hspace{0pt}uso\_\hspace{0pt}dias} mide días desde la última cita en los 90 días anteriores y queda nula cuando no hay uso en ese periodo. La fricción se resume con \texttt{bloqueos\_\hspace{0pt}90d}, que cuenta eventos \texttt{'Bloqueo'} en los 90 días previos y toma cero cuando no existen. El bloqueo es relevante ya que como se observa en los anexos \ref{tab:citas_bloq_act_rubro} y \ref{tab:citas_bloq_act_plan}  existe una relación directa entre el aumento del bloqueo y la disminución del uso del software. Se decide utilizar 90 días como periodo ya que debido a la naturaleza de los bloqueos, se obtiene una mayor cantidad de datos en 3 meses, ya que el bloqueo se da una sola vez al mes, por lo tanto un periodo menor no sería suficiente para el modelo.

\subsubsection{Definición de la etiqueta de baja a horizonte mensual}

La etiqueta \texttt{churn\_\hspace{0pt}T1} indica si un cliente activo en un corte presenta una baja efectiva en el mes siguiente. Para cada cliente se buscan eventos \texttt{'Baja'} entre \texttt{mes\_\hspace{0pt}corte} y el mismo día del mes siguiente; si hay varios, se toma la primera fecha. Luego se verifica si en ese mes aparece un \texttt{'Desbloqueo'} posterior a la baja. Si hay desbloqueo, la baja se considera revertida y no genera etiqueta; si no lo hay, se clasifica como baja efectiva. Así, \texttt{churn\_\hspace{0pt}T1} vale 1 cuando existe una baja efectiva en el mes siguiente y 0 en caso contrario, usando solo información disponible hasta el corte para las variables explicativas. Se definió el uso de un periodo igual a un mes debido a las necesidades del negocio y la estructura lógica que permite construir la base de datos. La suscripción es medida en mensualidades y se descarta una medida diaria debido al overfitting que genera esta decisión en el modelo.

\subsubsection{Tratamiento de valores faltantes y valores extremos}

Sobre el panel resultante se revisan valores faltantes y extremos. \texttt{rubro}, \texttt{plan\_\hspace{0pt}nombre} y \texttt{pais} presentan nulos que se imputan con una categoría explícita de desconocido. \texttt{arpa\_\hspace{0pt}proxy} tiene casos sin mensualidad válida; se imputan con la mediana por plan o la mediana global y se crea un indicador de imputación. En \texttt{recencia\_\hspace{0pt}uso\_\hspace{0pt}dias}, los nulos corresponden a ausencia de uso reciente, se imputan a 90 días y se marca esta situación con un indicador.

Para valores extremos se aplican recortes en percentiles altos. \texttt{arpa\_\hspace{0pt}proxy} se recorta y se transforma con
\[
\log\bigl(1 + \texttt{arpa\_\hspace{0pt}proxy}\bigr),
\]
manteniendo un indicador de recorte. \texttt{tenure\_\hspace{0pt}meses} se capa y se agrupa en bandas de antigüedad, además de considerar una transformación de raíz cuadrada sobre la versión capada. \texttt{citas\_\hspace{0pt}30d} se recorta y se transforma con
\[
\log\bigl(1 + \texttt{citas\_\hspace{0pt}30d}\bigr),
\]
junto con indicadores para actividad muy baja y para valores recortados.

\texttt{recencia\_\hspace{0pt}uso\_\hspace{0pt}dias} se capa en 90 días, se imputan nulos a ese valor y se generan bandas operativas y un puntaje continuo
\[
\texttt{fresh} = \frac{1}{1 + \texttt{recencia\_\hspace{0pt}uso\_\hspace{0pt}dias}}.
\]
\texttt{bloqueos\_\hspace{0pt}90d} se trata como conteo discreto pequeño, capando en tres y generando indicadores para al menos un bloqueo y para dos o más. Todas las imputaciones, recortes y transformaciones se definen usando solo información disponible hasta \texttt{mes\_\hspace{0pt}corte} y no utilizan la etiqueta \texttt{churn\_\hspace{0pt}T1}, de modo que la matriz final queda lista para el modelado sin fuga temporal.


\subsection{Fase 4: Modelado}

En esta fase se construyen los modelos que estiman el riesgo de baja mensual y que sirven de base para la segmentación conductual. El trabajo se organiza en cuatro tareas: selección del algoritmo, definición de la partición temporal y tamaño muestral, implementación del pipeline de preprocesamiento y entrenamiento con calibración de probabilidades.
...
En el conjunto de prueba el modelo calibrado alcanza un ROC–AUC cercano a 0{,}825 y una PR–AUC de aproximadamente 0{,}10 frente a una tasa base de 0{,}023, más de cuatro veces un ranking aleatorio. El primer decil de riesgo presenta una tasa de baja cercana a 11\,\% y concentra cerca del 48\,\% de los eventos; el 20\,\% superior captura alrededor del 68\,\% de las bajas, el 30\,\% cerca del 80\,\%, el 40\,\% en torno a 86–87\,\% y el 50\,\% superior acumula aproximadamente el 91\,\% de los eventos. Para el 20\,\% superior (punto de corte cercano a 0{,}043) el recall es cercano a 0{,}695 y la precisión a 0{,}081, con una tasa de eventos más de tres veces la base. Para un grupo más acotado de unos 600 clientes con mayor score, la precisión se aproxima a 0{,}148 y el recall a 0{,}228, configurando un segmento pequeño pero muy cargado de bajas para acciones intensivas.

\subsubsection{Calibración de probabilidades y estabilidad por segmentos}

La calibración en prueba muestra un Brier score cercano a 0{,}022, pendiente de calibración alrededor de 0{,}89, intercepto próximo a −0{,}30 y probabilidad media predicha (0{,}0229) muy similar a la observada (0{,}0234). La tabla de confiabilidad por grupos de probabilidad presenta concordancia razonable, con ligera subpredicción en la cola alta pero en márgenes manejables. Por plan, Básico y Reservo muestran buena alineación, mientras que Individual presenta una tasa real algo mayor que la estimada y Enterprise diferencias pequeñas con pocos casos. Por país, Chile mantiene una calibración muy ajustada y los mercados con baja muestra exhiben errores mayores por volumen limitado. Por rubro, Medicina/Salud y Centro de estética se calibran bien, y en Psicología/Psiquiatría la tasa real supera levemente la estimada, con errores en torno a 0{,}01, aceptables para uso operativo.

\subsubsection{Pruebas de robustez por país y rubro}

Las pruebas de robustez con el mismo modelo entrenado y calibrado muestran que, en segmentos de mayor volumen (global, Chile, Medicina/Salud, Chile más Medicina/Salud y Chile–México–Colombia), el ROC–AUC se mantiene en torno a 0{,}82–0{,}83, la PR–AUC es similar al valor global y el 20\,\% superior del ranking captura alrededor del 70\,\% de las bajas. En México, con menor número de observaciones, el ROC–AUC se sitúa cerca de 0{,}76, la PR–AUC triplica la tasa base y el top 20\,\% captura cerca del 70\,\% de los eventos con precisión sobre 11\,\%. En Colombia el ROC–AUC cae a valores en torno a 0{,}62, el 20\,\% superior captura solo cerca del 23\,\% de las bajas y la calibración muestra mayor error, lo que se atribuye al bajo volumen de datos. Se concluye que el modelo global es adecuado como referencia y que mejoras específicas para Colombia dependen de contar con más información.
...
Las decisiones de diseño del modelo se toman en un contexto donde la empresa requiere resultados accionables y comprensibles. Se elige regresión logística como algoritmo principal porque permite leer coeficientes y efectos de cada variable, lo que facilita que equipos no técnicos entiendan por qué un cliente tiene mayor probabilidad de baja y cómo se vincula ese riesgo con su uso de la plataforma, su plan y sus bloqueos, aun con un posible sacrificio de desempeño frente a modelos más complejos. El horizonte mensual T+1 se alinea con la gestión de cartera basada en facturación y decisiones comerciales mensuales, y la probabilidad T+3 derivada de T+1 entrega una señal adicional para acciones que requieren más tiempo sin entrenar un modelo distinto ni aumentar la complejidad operativa. Se adopta un modelo global para toda la región para reducir la complejidad y aprovechar información de todos los países, pero se mantiene abierta la opción de modelos por país o por rubro cuando la base sea mayor y se requiera más precisión en segmentos específicos. El prototipo se implementa con un flujo SQL, Colab y Excel que usa herramientas conocidas y evita cambios en infraestructura, aunque genera dependencia de la disciplina operativa y se plantea como trabajo futuro una integración directa con sistemas de la empresa y tableros de BI conectados a la salida del modelo.

\subsection{Impacto esperado en la operación y rol de Customer Success}

El modelo cambia la forma de priorizar la cartera al entregar probabilidades de baja y un ranking que permiten que Customer Success deje de tratar a todos los clientes activos de la misma manera y concentre tiempo en grupos de mayor riesgo, lo que obliga a revisar la asignación de horas, el número de contactos proactivos posibles al mes y las acciones adecuadas para cada segmento de valor. La segmentación conductual combina riesgo, valor económico y contexto comercial con los motivos de baja observados por rubro y plan para adaptar las intervenciones: en rubros dominados por cierres de local la capacidad de retención es menor, en rubros donde destacan competencia o costo beneficio un cliente en alto riesgo lleva a revisar propuesta de valor, precios y uso de funcionalidades frente a alternativas del mercado, de modo que un mismo nivel de probabilidad no tiene el mismo significado en todos los segmentos. El ranking de riesgo, los rangos cualitativos y los motivos de baja ofrecen un lenguaje compartido para Marketing, Ventas y Customer Success y facilitan campañas dirigidas a clientes de alto riesgo y alto valor o a clientes de riesgo medio con alto potencial de crecimiento. La probabilidad de baja no es el único criterio porque cada acción tiene un costo en tiempo, descuentos, visitas o beneficios, la memoria solo cuantifica el costo del churn y se propone avanzar hacia pilotos acotados que midan también el efecto y el costo de las acciones de retención y apoyen la revisión de las prácticas actuales de Customer Success para clientes que aparecen de forma recurrente en los tramos de mayor riesgo, considerando resultados numéricos y también contexto, carga operativa y cambios imprevistos del mercado.

\subsection{Comportamiento del cliente y límites del modelo}

El modelo se construye con variables de uso de la plataforma, bloqueos, antigüedad, plan, país y rubro, que capturan buena parte del comportamiento observable pero no toda la realidad del cliente, porque algunas decisiones de abandono responden a motivos subjetivos o estratégicos como cambios en la administración del centro, preferencias personales por otra herramienta, recomendaciones de colegas o situaciones económicas internas, por lo que pueden aparecer bajas repentinas en clientes que el modelo clasifica como de riesgo bajo y se mantiene un componente inevitable de imprevisibilidad. Los motivos de baja registrados por los ejecutivos entregan contexto, aunque usan categorías amplias y su registro varía entre personas y en el tiempo, por lo que se emplean para orientar políticas por rubro y plan pero no se incluyen como predictores directos del modelo para evitar amplificar sesgos y se mantienen como insumo cualitativo para la decisión comercial. Intentar representar todas las fuentes de variación con el conjunto actual de datos aumentaría el riesgo de sobreajuste al incorporar variables muy específicas o basadas en pocos casos, por lo que la memoria opta por un conjunto moderado de variables con sentido de negocio y recomienda mantener esa prudencia al incorporar nuevas fuentes de información. Entre estas fuentes aparece como prometedora la sistematización de visitas presenciales a los centros, hoy ausentes de las tablas y representadas como una columna sin datos, que en el futuro podría convertirse en un predictor relevante para ciertos rubros sin rediseñar el datamart, junto con el estudio de la competencia y de su oferta de software para interpretar mejor algunos patrones de churn observados solo desde datos internos.

\subsection{Limitaciones de datos y riesgos en el uso del prototipo}

El prototipo depende de la calidad de los registros de bajas y bloqueos, que definen la etiqueta de salida; errores en la codificación de estados, reversiones no registradas, demoras en la actualización o inconsistencias en fechas de inicio de pago, mensualidades o identificación de clientes activos al corte pueden distorsionar la estimación del riesgo, por lo que, aunque la memoria considera estas fuentes suficientemente confiables para el ejercicio, recomienda reforzar procesos de control interno, en especial si el modelo se integra de manera más automatizada en la operación. El flujo operativo actual exige varios pasos manuales, como ejecutar el script SQL con la fecha correcta, exportar el archivo, subirlo a Colab, correr el cuaderno y actualizar el Excel, y cada paso abre una posible fuente de error humano, de modo que se requiere designar responsables, documentar procedimientos y mantener rutinas de revisión para evitar que cambios menores en archivos o rutas interrumpan el uso del modelo. El entorno de Reservo es dinámico; nuevos competidores, cambios de plan, ajustes de precio o modificaciones en la propuesta de valor pueden alterar los patrones de churn y hacer que el modelo pierda vigencia si no se reentrena, por lo que el proyecto se limita a dejar herramientas y un prototipo funcional y el mantenimiento futuro dependerá de que la empresa incorpore el reentrenamiento y la gestión activa de estos riesgos en su ciclo de trabajo.
...
El primer objetivo, referido a la obtención de un conjunto validado de variables de comportamiento, se cumple a través del rendimiento del modelo de regresión logística. El conjunto formado por \texttt{arpa\_proxy}, \texttt{tenure\_meses}, \texttt{citas\_30d}, \texttt{recencia\_uso\_dias}, \texttt{bloqueos\_90d}, \texttt{plan\_nombre}, \texttt{pais} y \texttt{rubro} resulta suficiente para capturar información relevante sobre el riesgo de abandono: el valor de ROC–AUC cercano a 0,825 en el conjunto de prueba supera el umbral de 0,70 definido como indicador de éxito y deja estas variables como base para futuras extensiones del modelo.

El segundo objetivo, relativo a la construcción de un datamart estructurado, también se considera cumplido. El datamart mensual integra en una sola vista la información de clientes, uso, fricción y contexto, se genera mediante un único script parametrizable y alcanza una cobertura efectiva superior al 90\,\% de la cartera activa en los cortes utilizados. Este repositorio habilita tanto el análisis exploratorio de comportamiento como la estimación periódica de modelos predictivos.

En relación con el tercer objetivo, la definición y caracterización de segmentos, los resultados del modelo permiten agrupar a los clientes en rangos de riesgo Muy bajo, Bajo, Medio, Alto y Muy alto a partir de la probabilidad de baja estimada. En el conjunto de prueba, el 20\,\% de clientes con mayor score concentra alrededor del 70\,\% del churn observado, por encima del 40\,\% fijado como meta, y el cruce con los motivos de baja por rubro y plan muestra patrones distintos entre rubros dominados por cierre de local, competencia o costo beneficio. Esto confirma que los segmentos de alto riesgo están fuertemente asociados al abandono y que la segmentación conductual resultante es útil para el diseño de políticas focalizadas y diferenciadas según contexto.

El cuarto objetivo, referido al diseño de un portafolio de políticas por segmento, se cumple a nivel de propuesta. El proyecto entrega reglas de acción que combinan nivel de riesgo, valor económico aproximado y contexto comercial: contactos uno a uno para casos de Alto y Muy alto riesgo con alto pago mensual, campañas automatizadas para clientes de alto riesgo y menor valor, acciones centradas en costo beneficio y uso en rubros sensibles al precio, y monitoreo suave para riesgo Medio, junto con lineamientos específicos según los motivos de baja predominantes. La meta de tasa de aceptación de acciones igual o superior al 25\,\% aún no puede evaluarse porque no se ha ejecutado un piloto controlado, por lo que este indicador se mantiene como objetivo operativo para la fase de implementación.

El quinto objetivo, la implementación de un prototipo operativo del modelo analítico y de las políticas, también se considera alcanzado. La carpeta de uso para Customer Success, el cuaderno de scoring en Colab, el archivo de Excel de resultados y los tutoriales escritos y grabados conforman un flujo de trabajo completo y reproducible, que permite al equipo generar el datamart, aplicar el modelo sobre un corte mensual y actualizar la vista de resultados sin apoyo del estudiante. Además se realizaron pruebas en vivo del flujo y se dejó una primera versión del prototipo para ser utilizada y observada durante los meses siguientes, lo que traslada el modelo desde el ámbito académico a la práctica empresarial.

\subsection{Conclusiones generales}

El proyecto “Diseño de políticas comerciales para una empresa de tecnología para servicios de salud basado en el comportamiento de sus clientes” articula en una sola propuesta los componentes analíticos y operativos necesarios para que Reservo gestione de manera más proactiva la retención y la expansión de su cartera. A partir de un datamart mensual estructurado se construye un modelo de regresión logística calibrado que estima la probabilidad de churn a horizonte T+1 y permite derivar probabilidades a T+3, en coherencia con la dinámica mensual del negocio y con la necesidad de contar con señales tanto de corto como de mediano plazo.

Las métricas obtenidas muestran que el modelo discrimina con solidez entre clientes que se darán de baja y clientes que permanecerán activos, y que el 20\,\% de la cartera con mayor probabilidad estimada concentra cerca del 70\,\% de las bajas. Esta capacidad de focalización, combinada con la línea base económica levantada en la fase inicial, indica que reducciones moderadas en la tasa de bajas pueden traducirse en montos anuales de ingresos retenidos relevantes para la empresa y suficientes para justificar el esfuerzo de implementación y mantenimiento del sistema.
...
En conjunto, estos elementos permiten avanzar en el objetivo general de desarrollar un modelo de segmentación conductual que habilite políticas comerciales diferenciadas. El proyecto deja trazada una ruta para el trabajo posterior de la empresa: incorporar nuevas fuentes de información, como registros sistemáticos de visitas a los centros y métricas de satisfacción; profundizar en el análisis comparativo con la competencia; automatizar gradualmente el despliegue en la infraestructura propia; y diseñar pilotos que midan de manera conjunta la efectividad y el costo de las políticas de retención y expansión basadas en los scores del modelo. De este modo, Reservo queda en posición de transformar la analítica de churn en un componente estable de su estrategia comercial, utilizando el comportamiento de sus clientes y los motivos de baja observados como eje para decidir cómo, cuándo y con qué intensidad intervenir cada relación, y conectando de forma explícita las decisiones de Customer Success con el impacto económico observado en la cartera.
