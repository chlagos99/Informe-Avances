...
El sector de salud en Chile y América Latina vive un proceso sostenido de digitalización. El Observatorio de Desarrollo Digital de la CEPAL muestra que muchas empresas, en especial MIPYMES, siguen teniendo una presencia limitada, lo que refuerza la necesidad de soluciones que modernicen la gestión de servicios de salud \citep{CepalDDO2024}. Al mismo tiempo, el ecosistema regional de innovación registra más de un millar de empresas de salud digital, con foco en prevención, gestión y soporte a la toma de decisiones \citep{IDBLabHolonIQ2024}.
...
La pandemia aceleró estos procesos. La expansión de la telemedicina y de modelos híbridos de atención elevó las expectativas de pacientes y equipos de salud en torno a continuidad de cuidados, acceso remoto y coordinación entre prestadores \citep{SuperSalud2025}. Persisten, sin embargo, brechas de adopción y recursos que limitan el uso sostenido de estas herramientas, en especial en prestadores pequeños y medianos \citep{RevChilSalud2025}. En este contexto surge un ecosistema de software competitivo, donde se distinguen tres tipos de actores: Startups y empresas nativas digitales, ágiles e innovadoras, enfocadas en nichos específicos, pero con desafíos para escalar y generar confianza de largo plazo. Empresas locales consolidadas, con conocimiento del marco regulatorio y cultural chileno y bases de clientes estables, aunque con procesos de innovación más lentos. Actores internacionales, con tecnologías maduras y marcas globales, que deben adaptar sus soluciones a las particularidades normativas del país, como los requisitos de seguridad y privacidad establecidos en la Ley de Derechos y Deberes del Paciente \citep{Ley20584}.
...
El flujo general del cliente en Reservo abarca el ciclo de vida completo, desde el contacto inicial con el área de Marketing hasta su permanencia o pérdida definitiva (anexo \ref{img:proceso}). El proceso integra las etapas de captación y venta, gestionadas por SDR y ejecutivos comerciales, seguidas por la incorporación a la plataforma mediante el Onboarding a cargo de Customer Success. El recorrido continúa con la atención de Soporte y la gestión de Retención, fases donde se define la continuidad del servicio o la ocurrencia de la fuga.

\subsubsection{Etapa de Captación}

En la etapa de captación, ilustrada en el anexo~\ref{img:captacion}, Marketing ejecuta campañas que transforman a un visitante desconocido en un \textit{lead} registrado. El equipo de \textit{Ventas SDR} contacta a ese lead, valida que cumpla el perfil objetivo y, si la calificación es positiva, lo convierte en \textit{lead calificado}. Finalmente, el SDR agenda una demostración con un ejecutivo de ventas y envía una cotización, momento en que el contacto pasa a estado de \textit{Oportunidad de venta} y queda listo para la etapa comercial.

\subsubsection{Etapa de Ventas}

El proceso de ventas, descrito en el anexo~\ref{img:etapa_venta}, comienza cuando la oportunidad asiste a la demo del producto. Si decide contratar, el ejecutivo configura la cuenta en el sistema y el estado cambia a \textit{precliente}. Tras el pago inicial, el registro pasa a \textit{cliente} y se traspasa formalmente al equipo de \textit{Customer Success}, que inicia el Onboarding y cierra la etapa comercial.

\subsubsection{Etapa de Onboarding}

En el Onboarding, detallado en el anexo~\ref{img:etapa_onboarding}, Customer Success prepara la cuenta, agenda una primera capacitación y monitorea el uso inicial. Según este comportamiento, el cliente puede ser clasificado como de \textit{riesgo alto} o \textit{riesgo medio}, lo que inicia contactos proactivos para asegurar la adopción. Si no se observan alertas en un periodo definido, el Onboarding se considera exitoso y el cliente pasa al uso regular de la plataforma.

\subsubsection{Etapa de Cliente Activo}

La etapa de \textit{cliente activo}, ilustrada en el anexo~\ref{img:etapa_cliente_activo1}, se centra en retención y expansión. Customer Success monitorea indicadores de salud de la cuenta y activa acciones de seguimiento cuando detecta bajo uso u otros signos de riesgo. En casos sanos se buscan oportunidades de \emph{upselling}; si el cliente acepta la oferta y paga, se actualiza su plan y se refuerza su condición de \textit{cliente vigente}. En paralelo se gestionan tickets de soporte y procesos de facturación, donde problemas de pago recurrentes pueden terminar en la pérdida del cliente.

\subsubsection{Etapa de Cliente Inactivo}

El flujo de \textit{cliente inactivo}, descrito en el anexo~\ref{img:etapa_cliente_inactivo}, comienza cuando un cliente solicita cancelar el servicio. Customer Success procesa la baja \footnote{En esta memoria, los conceptos de churn, baja, abandono y deserción se utilizan como sinónimos. No obstante, existe una distinción técnica: churn y deserción refieren a indicadores métricos y tasas de salida de la cartera. En contraste, baja y abandono describen la acción o el evento específico que pone término a la relación comercial.}, registra el motivo y formaliza el estado de \textit{dado de baja}. El diagrama contempla también la reactivación: si un cliente decide volver, se gestiona un nuevo contrato y se reactiva su cuenta, devolviéndolo al estado de \textit{cliente activo} dentro del ciclo general.
...
\begin{itemize}
    \item \textit{Pérdida recurrente de clientes e ingresos.} La empresa presenta una \textit{tasa de churn} mensual de \textit{2{,}46\,\%}, equivalente a \textit{70 bajas} en lo que va de 2025\footnote{La \textit{Tasa de Churn} se define en la tabla~\ref{tab:estados-cliente}. El valor y el número de bajas corresponden a datos internos para 2025. Fuente: Base de datos de Reservo.}. Desde enero de 2022 el churn promedia \textit{2{,}77\,\%}. El anexo ~\ref{img:churn3anios} muestra su evolución junto con el número de clientes al inicio de cada mes (anexo~\ref{img:clientes_por_mes}). Aunque la serie presenta una ligera tendencia a la baja, el indicador se mantiene desde hace casi dos años en el rango 2–3\,\% sin bajar del 2\,\%. Esto es relevante porque los benchmarks para empresas SaaS recomiendan apuntar a menos de 2\,\% mensual para alinearse con las mejores prácticas del mercado \citep{ChartMogul2023ChurnRate}.

    \item \textit{Expansión y reactivación no sistemáticas.} La empresa genera valor adicional, pero sin un crecimiento acelerado. En 2024 se registraron \textit{57 upsellings} y en 2025 se han concretado \textit{60} hasta la fecha. Respecto de la reactivación, en 2024 se recuperaron \textit{100 clientes} y en 2025 van \textit{89}\footnote{Datos de upsellings y reactivaciones según registros internos. Fuente: Base de datos de Reservo.}. Las cifras indican un avance moderado en expansión y una posible pérdida de fuerza en reactivación, lo que sugiere ausencia de una estrategia proactiva y repetible.

    \item \textit{Asignación ineficiente de recursos de postventa.} El equipo de Customer Success suele intervenir tarde y con ofertas poco ajustadas al perfil del cliente en riesgo. No existe una clasificación operativa de riesgo durante la etapa activa (más allá del Onboarding), por lo que muchas acciones se ejecutan cuando la probabilidad de abandono ya es alta. Se termina priorizando la contención de clientes al borde de la fuga y se desaprovechan intervenciones tempranas con mayor potencial de retención o expansión.
\end{itemize}

\subsubsection{Análisis y elección del problema central}

Desde esta perspectiva, la tasa de abandono elevada, el crecimiento limitado del valor y la baja efectividad de postventa no son causas de fondo, sino consecuencias de una falla operativa más profunda. Se busca entonces el problema que, al abordarse, tenga mayor impacto sobre los demás síntomas. El problema central seleccionado es:

\begin{center}
    \textit{\textit{Insuficiente caracterización y segmentación conductual de la cartera activa en postventa}}
\end{center}
...
\begin{itemize}
    \item \textit{Decisiones basadas en indicadores de resultado.} Las estrategias se activan a partir de métricas retrospectivas, como la \textit{tasa de churn} o un \textit{NPS} de \textit{52}\footnote{El \textit{NPS} se define en la tabla~\ref{tab:estados-cliente}. El valor corresponde al último resultado medido por la empresa. Fuente: Base de datos de Reservo.}. Estos indicadores describen lo que ya ocurrió, pero no permiten diferenciar el tratamiento entre clientes antes de que el riesgo se materialice.

    \item \textit{Análisis estático de la información.} Se observa el estado actual de cada cliente, pero no su trayectoria en el tiempo. La ausencia de una mirada histórica estandarizada dificulta detectar señales tempranas, como una baja gradual en el uso de la plataforma.

    \item \textit{Falta de criterios conductuales para segmentar.} No se han definido patrones de uso ajustados a las diversas características del cliente, que distingan clientes saludables, en riesgo o con potencial de expansión. Sin estos criterios, la segmentación sigue siendo principalmente demográfica o comercial y no captura el comportamiento dinámico.

    \item \textit{Enfoque operativo reactivo.} El trabajo de Customer Success se organiza para responder a eventos negativos (quejas, impagos, bloqueos) más que para anticiparse al riesgo. No existe un esquema sistemático que combine datos de comportamiento y valor económico para priorizar acciones personalizadas sobre la cartera activa.
\end{itemize}
...
Las razones estratégicas se basan en dos pilares principales:  
\begin{itemize}
    ...
    \item Optimizar el esfuerzo de Customer Success. Un trato homogéneo de toda la cartera genera sobrecostos y baja efectividad. La segmentación sistematizada basada en datos permite distinguir clientes de mayor riesgo o mayor potencial y asignar prioridades claras de trabajo. Esto facilita organizar la agenda del equipo, concentrar los esfuerzos donde el impacto esperado es mayor y se alinea con prácticas de gestión de relaciones con clientes \citep{Kumar2010}.
\end{itemize}
...
\begin{itemize}
    \item \textit{Ingreso Promedio (ARPA):} \$58.700 CLP.
    \item \textit{Churn mensual ($c$):} 2,4\% ($0{,}024$).
    \item \textit{Tasa de descuento anual ($WACC$):} 10\% ($i \approx 0{,}008$ mensual).
    \item \textit{Margen Bruto ($m$):} 70\% ($0{,}70$). Este valor corresponde al estándar de la industria SaaS B2B, donde los costos directos de venta (servidores y soporte) son bajos respecto al ingreso recurrente.
    \item \textit{Margen Neto Estimado:} 10\% ($0{,}10$). Se utiliza como referencia conservadora para estimar la utilidad final de la empresa.
\end{itemize}
...
\begin{itemize}
    \item \textit{Pesimista:} Capacidad baja de gestión. Se retienen 20 clientes por trimestre (80 clientes anuales).
    \item \textit{Neutro:} Capacidad media. Se retienen 30 clientes por trimestre (120 clientes anuales).
    \item \textit{Optimista:} Capacidad alta. Se retienen 40 clientes por trimestre (160 clientes anuales).
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}
    \hline
    \textit{Escenario} & \textit{Retención Anual} & \textit{Valor Preservado (CLV)} \\ \hline
...
La segunda tarea consiste en definir las variables que describen a cada cliente en cada mes de corte. Estas variables se calculan en el script principal y se detallan en el documento de variables del datamart. 
...
En esta fase se define el uso operativo de los resultados del modelo mediante un prototipo que conecta el análisis con la gestión comercial. El diseño integra el flujo mensual de scoring, los artefactos entregados, la segmentación conductual y los lineamientos para el equipo de Customer Success.

\subsubsection{Flujo mensual de scoring sobre la cartera actualizada}

El flujo operativo comienza en la base de datos transaccional. Para actualizar el modelo, se ejecuta el script \texttt{Datamart Principal Memoria\_Act.sql} sobre el esquema \texttt{reservov2} mediante DataGrip y un túnel SSH. El usuario debe ajustar el parámetro \texttt{@fin} con el primer día del mes siguiente al periodo analizado. Este proceso genera una tabla en formato cliente–mes que contiene todas las variables necesarias para la ejecución del modelo.

El resultado se exporta como \texttt{database\_act.csv} y se carga en Google Colab junto a la base histórica \texttt{database1.csv} (anexo \ref{img:colb_cs}). El cuaderno \texttt{Colab\_para\_Customer\_Success.ipynb} aplica el modelo calibrado para calcular probabilidades de baja, asignar rangos de riesgo y generar el ranking global. Finalmente, el usuario actualiza el archivo Excel \texttt{Clientes en riesgo de fuga - modelo} mediante Power Query para visualizar los resultados. Este flujo asegura una transición directa desde los datos crudos hasta la herramienta de toma de decisiones.

\subsubsection{Artefactos operativos y carpeta de uso para Customer Success}

Para facilitar el despliegue, se entrega la Carpeta de uso Customer Success (anexo \ref{img:carp_cs}), la cual centraliza el script SQL, el cuaderno de modelado y los archivos de datos. El script SQL permite regenerar el datamart mensual de forma parametrizada sin necesidad de escribir código adicional. Por su parte, la carpeta \texttt{Colab py} encapsula el pipeline de preprocesamiento y el módulo de inferencia, separando la etapa de entrenamiento del uso diario para simplificar la operación por parte del equipo.

La carpeta Excel contiene el archivo final de explotación configurado para leer automáticamente las salidas del modelo (anexo \ref{img:xls_res}). El libro presenta tablas y vistas filtrables por país, plan, rubro y ranking, lo que permite a Customer Success priorizar la gestión de clientes sin interactuar con código ni bases de datos. De esta forma, el conjunto de artefactos garantiza la autonomía operativa y la reproducibilidad del sistema de alerta temprana dentro de la empresa.
...
La salida del modelo define una propuesta de segmentación basada en combinaciones de riesgo de baja y valor económico aproximado. Se utilizan las probabilidades de fuga mensual y trimestral para identificar grupos de prioridad, como aquellos de alto riesgo y alto valor. El valor económico se estima mediante la variable \texttt{arpa\_proxy}, la antigüedad y el plan contratado. Esta estructura permite ubicar a los clientes en bandas operativas adaptadas al país y rubro, facilitando la toma de decisiones sin realizar cálculos adicionales en las herramientas de negocio.

La implementación optimiza el trabajo de los cinco integrantes de Customer Success al reducir el tiempo de revisión manual en un 90\,\% y el tiempo total de contacto en un 50\,\%. Al disponer de los factores de riesgo ya procesados, el equipo aumenta su capacidad para generar ofertas personalizadas, con una efectividad de retención estimada del 75\,\% en casos gestionables. Estos beneficios operativos permiten transformar la gestión de postventa en un proceso proactivo basado en la priorización de la cartera activa.

El análisis de motivos de baja históricos por rubro y plan permite refinar las acciones de retención. Factores como el cierre de local, los bloqueos de 40 días y la competencia presentan pesos distintos según el perfil del cliente. Por ejemplo, en Odontología la competencia es el motivo principal (23\,\%), mientras que en el plan Enterprise los problemas de software (19\,\%) y requisitos sanitarios (11\,\%) tienen mayor importancia. Estas diferencias indican que una misma probabilidad de baja requiere respuestas diferenciadas según el contexto comercial.

Las políticas de intervención se ejecutan según el decil de riesgo. No se interviene en segmentos de riesgo bajo, mientras que los casos de riesgo muy alto se consideran generalmente perdidos, salvo excepciones de alto valor o competencia. El foco de gestión se concentra en los riesgos alto y medio, donde existe un margen de maniobra de hasta tres meses. Las acciones incluyen contactos personalizados para cuentas de alto valor, seguimiento de bloqueos técnicos y demostraciones de funcionalidades para fomentar la expansión del uso del software.
...
\begin{table}[H]
  \begin{threeparttable}
    ...
    \begin{tabular}{|l|C{3.5cm}|C{3.3cm}|C{5.5cm}|}
      \hline
      \textit{Estado} & \textit{Definición} & \textit{Métricas asociadas} & \textit{Fórmula / Cálculo} \\
      \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|C{3.2cm}|C{3.3cm}|C{3.5cm}|}
        \hline
        \textit{Rubro} & \textit{Número de bloqueados} & \textit{Prom. citas últimos 3 meses (bloq.)} & \textit{Prom. citas último año (act.)} \bigstrut\\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|C{3.5cm}|C{3.3cm}|C{5.5cm}|}
        \hline
        \textit{Plan} & \textit{Número de bloqueados} & \textit{Prom. citas últimos 3 meses (bloq.)} & \textit{Prom. citas último año (act.)} \bigstrut\\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{l r}
        \hline
        \textit{Rubro} & \textit{Promedio de mensualidad} \bigstrut\\
        \hline
        ...
        Odontología & \$ 40.950 \\
        \hline
        \textit{Total general} & \textit{\$ 58.700} \bigstrut[b]\\
        \hline
    \end{tabular}
