...
El desarrollo del proyecto permite dar por cumplido el objetivo general de dotar a Reservo de un sistema analítico y operativo capaz de gestionar la retención de clientes de manera proactiva. En primera instancia, la validación de variables de comportamiento confirmó que la información transaccional interna es suficiente para predecir el riesgo de abandono sin necesidad de recurrir a fuentes externas. El conjunto de indicadores seleccionados, que incluye métricas de valor monetario aproximado, antigüedad, frecuencia de uso reciente y fricción operativa por bloqueos, demostró una capacidad predictiva robusta. El rendimiento del modelo superó holgadamente los umbrales de éxito definidos al inicio del estudio, situándose en un nivel de discriminación elevado que valida estadísticamente la relación entre estos patrones de conducta y la probabilidad de fuga futura.

En cuanto a la infraestructura de datos, se cumplió el objetivo de construir un datamart estructurado que consolida la visión del cliente en una sola tabla mensual. Este repositorio, generado mediante un script único y parametrizable, alcanza una cobertura efectiva superior al 90\,\% de la cartera activa en los cortes analizados. La integración de dimensiones de facturación, uso de la plataforma y contexto comercial en una vista unificada no solo alimenta el modelo predictivo actual, sino que habilita al equipo de Customer Success para realizar análisis exploratorios profundos y monitorear la salud de la base de clientes de forma continua y autónoma, resolviendo la carencia de información centralizada que existía previamente.

Respecto a la caracterización de la cartera, la segmentación conductual derivada del modelo permitió agrupar a los clientes en rangos de riesgo diferenciados, desde niveles muy bajos hasta críticos. La capacidad del sistema para concentrar cerca del 70\,\% del abandono real observado dentro del 20\,\% de los clientes con mayor puntaje de riesgo confirma la utilidad práctica de la herramienta para focalizar recursos. Además, el cruce de estos segmentos con los motivos de baja históricos reveló patrones comerciales distintivos; se evidenció que en ciertos rubros predominan causas estructurales como el cierre de local, mientras que en otros el riesgo está traccionado por la competencia o la sensibilidad al precio. Este hallazgo aporta una capa de inteligencia de negocio fundamental, indicando que una misma probabilidad de baja requiere interpretaciones distintas según el perfil del cliente.

El diseño de un portafolio de políticas comerciales se materializó en una propuesta de reglas de acción que responden a esta segmentación. Las políticas definidas combinan el nivel de riesgo detectado con el valor económico y el contexto del cliente, estableciendo estrategias diferenciadas: contactos personalizados uno a uno para cuentas de alto valor en riesgo, campañas digitales automatizadas para segmentos masivos y un monitoreo preventivo liviano para casos de riesgo medio. Si bien la tasa de aceptación de estas acciones se mantiene como un indicador a evaluar en fases de operación continua, los lineamientos entregados proporcionan una guía clara para que el equipo de postventa reemplace la intuición por estrategias basadas en evidencia.

La implementación del prototipo operativo marcó el cumplimiento del objetivo de transferencia tecnológica. La entrega de una carpeta de uso completa —que incluye los scripts de procesamiento, los cuadernos de cálculo de riesgo en la nube, las herramientas de visualización en Excel y los tutoriales de soporte— asegura que el flujo de trabajo sea reproducible por el equipo de Customer Success sin dependencia externa. Las pruebas realizadas validaron que el equipo puede generar el datamart, aplicar el modelo sobre un corte mensual y actualizar la vista de resultados de manera autónoma, trasladando efectivamente el desarrollo desde un ejercicio académico a una capacidad instalada en la empresa.

Finalmente, la integración de todos estos componentes sustenta la validación de la hipótesis económica del proyecto bajo el enfoque de valor de vida del cliente (CLV). Los análisis de impacto financiero demuestran que la capacidad de focalizar los esfuerzos de retención en los segmentos correctos permite preservar un valor patrimonial significativo que de otro modo se perdería. En un escenario de efectividad operativa media, se estima que el modelo contribuye a retener activos por un monto cercano a los \$154 millones anuales. Esta cifra confirma que la gestión analítica del churn trasciende la mejora operativa y se constituye como una palanca de rentabilidad crítica, justificando plenamente el esfuerzo de implementación y posicionando a la retención como un eje central de la estrategia comercial de Reservo.
