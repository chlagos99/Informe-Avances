...
\def\documentparskip {6}           % Tamaño adicional entre párrafos (+/-) [pt]
\def\fontdocument {mathptmx}        % Tipografía base, ver soportadas en manual
...
\def\portraitstyle {style5}        % Estilo portada (21 estilos)
...
\def\indexstyle {ft}               % Tipo {f:figura,t:tabla,c:código,e:ecuación}
...
\def\sssectionfontsize {\huge}    % Tamaño fuente subsubsection
...
\def\urlcolor {black}            % Color de los enlaces web (\href,\url)

% MÁRGENES DE PÁGINA
\def\firstpagemargintop {3}      % Margen superior página portada [cm]
\def\pagemarginbottom {3}        % Margen inferior página [cm]
...
% NOMBRE DE OBJETOS
\def\nameabstract {Diseño de políticas comerciales para una empresa de tecnología para servicios de salud basado en el comportamiento de sus\\ clientes}           % Nombre del resumen-abstract
